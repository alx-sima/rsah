        ...
        if let Some(click_start) = state.click {
            if let Some(click_end) = input::get_mouse_square(ctx, state.guest) {
                if click_start == click_end {
                    your_turn(state, (click_end.1, click_end.0));
                }
                // else drag & drop??
            }
        }
...
fn your_turn(state: &mut State, click: Pozitie) {
    ...
    // Daca nu este, selecteaza alta piesa (daca e de aceeasi culoare).
    } else if let Some(piesa) = &state.tabla.at(click).piesa {
        if piesa.culoare == state.turn && Some(click) != state.piesa_sel {
